...
if(!defined('lang')){
	...
	if(sizeof($active_path)>0){
		...
		$language = $lang->getLanguages($active_path[0]);
		// если первая активная секция является языковым идентификатором - фиксируем это и переназначаем корневой путь url
		if($language){

			define('lang',$language['strid']);
			$url->setRootPath(path($url->getRootPath(),$language['strid']));
	...
	if(!$language){
		...
		$burl->set(loc);
		$burl->setPath(path(rvdir_admin,$language['strid']));
		$burl->redirect();
        $burl->clear();
...
define('active_base',path(admin_base,lang).'/');
...
if(!$cur_admin){ // если мы не авторизованы как админ - требуется авторизация админа
	...
	if(sizeof($menu_modules_tmp)>0){
		...
		foreach($menu_modules_tmp as $i=>$mod){
			...
			if(isset($mod['use_in_back']) && $mod['use_in_back'] == 1 && $mod['path'] != 'default'){
				...
				$mod['back_link'] = path(rvdir_admin,lang,$mod['path'].'/');
    ...
    if(!$view->getActiveSectionByIndex(0) && $view->getDefaultModuleName()){
        ...
        $burl->set(loc);
        $burl->setPath(path(rvdir_admin, lang, $view->getDefaultModuleName()));
        $burl->redirect();
        $burl->clear();
	...
	$view->includeScript('/js/jquery.js');
	$view->includeScript(path('/'.cmsBase, '/side/back/js/jquery-ui/jquery-ui.min.js'));
	$view->includeScript('/js/jquery.scrollTo.min.js');
	$view->includeScript('/js/jquery.timers.min.js');
	$view->includeScript('/js/global.js');
	...
	if($module_info['use_in_back'] && isset($module_info['fpath']) && !empty($module_info['fpath'])){
		...
		if(file_exists($include_back_module)){
			...
			define('mod_back_url',path(loc,rvdir_admin,lang,$module_info['path']));
...
//            print_r($tpl->getTemplateVars());
            ...
            if(isset($_REQUEST['form_action']) && !empty($_REQUEST['form_action'])){
                ...
                else $m->mess('Не удалось подключить обработчик форм!', 'e');
	...
	// Рендринг шаблона
	$view->render();
	$debug->timeMarker('Рендринг шаблона');
